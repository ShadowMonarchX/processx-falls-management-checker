...
def test_peter_parker_day1_flags_are_specific():
    ...
    flags = engine.analyze(note)
    assert any(flag.field == "Pain scale" for flag in flags)
    assert any(flag.field == "Vital signs" for flag in flags)
    assert any(flag.field == "GP notification time" for flag in flags)
    assert any(flag.field == "NOK details" for flag in flags)
