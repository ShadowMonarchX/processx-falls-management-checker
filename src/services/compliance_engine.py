...
class ComplianceEngine:
    ...
    def _check_day1_fall_details(self, note: ResidentNoteModel, rule: PolicyRuleModel, text: str) -> list[ComplianceFlagModel]:
        ...
        if not any(phrase in text for phrase in ["room", "corridor", "bathroom", "activity room", "near", "bedside", "wardrobe", "bed", "chair", "floor"]):
        ...
        if not any(phrase in text for phrase in ["witnessed", "not witnessed", "unwitnessed"]):
    ...
    def _check_day1_assessment(self, note: ResidentNoteModel, rule: PolicyRuleModel, text: str) -> list[ComplianceFlagModel]:
        ...
        if not any(phrase in text for phrase in ["alert", "orientated", "oriented", "conscious", "unconscious"]):
        ...
        if not any(phrase in text for phrase in ["visible injury", "no visible injury", "laceration", "bruise", "bruising"]):
        ...
        if not any(phrase in text for phrase in ["able to move all limbs", "move all limbs", "lift right arm", "weight-bear", "mobility", "move all limbs on request"]):
    ...
    def _check_day1_actions(self, note: ResidentNoteModel, rule: PolicyRuleModel, text: str) -> list[ComplianceFlagModel]:
        flags: list[ComplianceFlagModel] = []
        if not any(phrase in text for phrase in ["immediate actions", "assisted", "call bell", "comfort"]):
        ...
        if "gp" in text and not any(phrase in text for phrase in ["notified at", "called at", "time"]):
        ...
        if any(phrase in text for phrase in ["advised to monitor", "monitor"]) and not any(phrase in text for phrase in ["x-ray", "transfer", "threshold", "if pain worsens", "review", "monitor pain and mobility", "monitoring"]):
        ...
        if any(phrase in text for phrase in ["family has been informed", "next of kin", "nok"]) and not any(phrase in text for phrase in ["notified at", "called at"]):
        ...
        if "care plan" in text and any(phrase in text for phrase in ["will update", "plan to update", "tbd"]):
        ...
        if not any(phrase in text for phrase in ["falls risk score", "risk score", "low", "medium", "high"]):
    ...
    def _check_day2(self, note: ResidentNoteModel, rule: PolicyRuleModel, text: str) -> list[ComplianceFlagModel]:
        flags: list[ComplianceFlagModel] = []
        if not re.search(r"\b\d{1,2}\s*/\s*10\b", text) and not any(phrase in text for phrase in ["improved", "worsened", "resolved"]):
        ...
        elif any(phrase in text for phrase in ["seems okay", "doing much better", "better", "mobilising"]) and not re.search(r"\b\d{1,2}\s*/\s*10\b", text):
        ...
        if not any(phrase in text for phrase in ["weight-bear", "weight bear", "full weight-bearing", "full weight bear"]):
        ...
        elif not any(phrase in text for phrase in ["without pain", "full weight-bearing", "full weight bear"]):
        ...
        if not any(phrase in text for phrase in ["new symptoms", "no new symptoms", "bruising", "swelling", "confusion", "behaviour"]):
        ...
        if not any(phrase in text for phrase in ["bp", "hr", "rr", "temp", "temperature", "spo2", "vitals"]):
        ...
        if not any(phrase in text for phrase in ["gp", "x-ray", "reviewed", "follow-up", "actioned"]):
    ...
    def _check_day3(self, note: ResidentNoteModel, rule: PolicyRuleModel, text: str) -> list[ComplianceFlagModel]:
        ...
        elif any(phrase in text for phrase in ["doing much better", "feeling well", "better"]) and not any(phrase in text for phrase in ["resolved", "0/10"]):
        ...
        if not any(phrase in text for phrase in ["baseline", "returned to baseline", "full weight-bearing", "weight bearing"]):
        ...
        if not any(phrase in text for phrase in ["outstanding actions", "no outstanding", "complete", "resolved"]):
        ...
        if not any(phrase in text for phrase in ["incident closed", "monitoring period complete", "formally closed", "complete"]):
        ...
        if not any(phrase in text for phrase in ["falls prevention plan", "care plan", "reviewed", "discussed"]):
        ...
        if not any(phrase in text for phrase in ["escalate", "escalation", "no further follow-up"]):
